          ...
          <div className={`container ${s.cv_section}`}>
            ...
            <div className={s.experience_block}>
              <div className={s.exp_header}>
                {teamMember.experience && (
                  <>
                    <h3>
                      Work experience <span>{teamMember.experience} years</span>
                    </h3>
                    <p>{teamMember.workYears}</p>
                  </>
                )}
              </div>
              {teamMember.experiences &&
                teamMember.experiences.jobs &&
                teamMember.experiences.jobs.map((job, index) => (
              ...
              <div className={s.about}>
                ...
                <div className={s.skills_block}>
                  ...
                  <div className={s.skills}>
                    {teamMember.skills &&
                      teamMember.skills.hard &&
                      teamMember.skills.hard.map((skill, index) => (
                        <p key={index} className={s.hard}>
                          {skill}
                        </p>
                      ))}
                    {teamMember.skills &&
                      teamMember.skills.soft &&
                      teamMember.skills.soft.map((skill, index) => (
                        <p key={index} className={s.soft}>
                          {skill}
                        </p>
                      ))}
                  </div>
            ...
            <div className={s.more_info}>
              <div className={s.advanced}>
                <h2>
                  <span>Advanced</span> skills
                </h2>
                <div>
                  <div className={s.adv_blocks}>
                    <h3>Languages</h3>
                    <div className={s.languages}>
                      {teamMember.advancedSkills &&
                        teamMember.advancedSkills.advancedLanguages &&
                        teamMember.advancedSkills.advancedLanguages.map(
                          (lang, index) => (
                            <p key={index}>
                  ...
                  {teamMember.advancedSkills &&
                    teamMember.advancedSkills.advancedEducation &&
                    teamMember.advancedSkills.advancedEducation.length > 0 && (
                      <div className={s.adv_blocks}>
                        <h3>Education</h3>
                        <div className={s.education}>
                          {teamMember.advancedSkills.advancedEducation.map(
                            (edu, index) => (
                              <div key={index}>
                                <p>{edu.description}</p>
                                <span>{edu.p}</span>
                  ...
                  {teamMember.advancedSkills &&
                    teamMember.advancedSkills.additionalEducations &&
                    teamMember.advancedSkills.additionalEducations.length >
                      0 && (
                      <div className={s.adv_blocks}>
                        <h3>Additional education</h3>
                        <div className={s.education}>
                          {teamMember.advancedSkills.additionalEducations.map(
                            (edu, index) => (
                              <div key={index}>
                                <p>{edu.education}</p>
                                <span>{edu.p}</span>
